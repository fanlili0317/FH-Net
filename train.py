...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/FSSD-12/1shot/fold0_train.yaml', help='config file')
...
def main_worker(argss):
    ...
    if not args.save_path or not str(args.save_path).strip():
        raise ValueError("save_path must be a non-empty directory in the training configuration.")
    ...
    optimizer = model.get_optim(model, args, args.base_lr)
