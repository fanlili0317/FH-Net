...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/FSSD-12/1shot/fold0_test.yaml', help='config file')
...
def main_worker(argss):
    ...
    if not args.weight or not str(args.weight).strip():
        raise ValueError("weight must point to a trained FH-Net checkpoint in the test configuration.")
    if not os.path.isfile(args.weight):
        raise FileNotFoundError("FH-Net checkpoint not found at '{}'.".format(args.weight))
...
def validate(val_loader, model, criterion):
    ...
    log_interval = max(1, test_num // 100)
    assert test_num % args.batch_size_val == 0
    iter_num = 0
    total_time = 0
    for e in range(1):
        for i, (input, target, s_input, s_mask, subcls, ori_label, query_img_name) in enumerate(val_loader):
            ...
            if args.is_save:
                ...
                if len(pred.shape) == 3 and pred.shape[0] in [1, 3]:
                ...
                save_path = r'{}/{}'.format(args.save_path, query_img_name[0].replace('jpg', 'png'))
