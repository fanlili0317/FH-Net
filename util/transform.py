...
class RandScale(object):
    ...
    def __call__(self, image, label):
        ...
        assert isinstance(image, np.ndarray) and image.size > 0
        ...
        label = np.array(label)  # 确保是 ndarray
        if label.dtype not in [np.uint8, np.int32]:
            label = label.astype(np.int32)
        assert isinstance(label, np.ndarray) and label.size > 0
        ...
        # print(f"Image type: {type(image)}, shape: {image.shape}, dtype: {image.dtype}, flags: {image.flags}")
