...
def make_dataset(split=0, data_root=None, data_list=None, train_class=None, mode=None):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        #     gt_name = gt_name.replace('jpg', 'png')
        # else:
        #     gt_name = gt_name.replace('png', 'jpg')
...
class SemData(Dataset):
    def __init__(self, split=0, shot=1, data_root=None, data_list=None, transform=None, mode='train'):
        ...
        if self.split == 2:
            ...
            '''FSCSSD设置'''
            # self.train_class = list(range(1, 13))
            # self.val_class = list(range(13, 16))
        ...
        if self.mode == 'train':
            self.img_gt_class_list, self.class_img_gt_dict = make_dataset(split, data_root, data_list, self.train_class, self.mode)
            assert len(self.class_img_gt_dict.keys()) == len(self.train_class)

        elif self.mode == 'val':
            self.img_gt_class_list, self.class_img_gt_dict = make_dataset(split, data_root, data_list, self.val_class,  self.mode)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            return query_rgb, query_mask, s_x, s_y, subcls_list, raw_label, os.path.basename(query_img)
